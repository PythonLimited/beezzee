...
Usage:
    python scripts/train_mtc.py                    # uses default config
    python scripts/train_mtc.py --config qwen3_6_27b_gpu
"""
...
def ensure_model_local(cfg: TrainConfig):
    ...
    if not (local / "config.json").exists():
        ...
        snapshot_download(
            cfg.model_id, local_dir=str(local), local_dir_use_symlinks=False
        )
    else:
...
def main():
    ...
    parser.add_argument("--profile", action="store_true", help="Show per-step timing")
    ...
    for step in range(cfg.steps):
        ...
        if (step + 1) % cfg.eval_every == 0:
            ...
            if args.profile and t_count > 0:
                n = cfg.eval_every
                print(
                    f"           ── profile ({t_count} evals, {cfg.eval_every} steps each) ──\n"
